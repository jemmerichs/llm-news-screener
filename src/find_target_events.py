...
class FindTargetEvents:
    ...
    def get_llm_events(self, n=3) -> List[TrackedEvent]:
        ...
        prompt = (
            f"Top {n} upcoming financial events in next 1-2 wks?"
        )
        try:
            response = self.client.responses.parse(
                ...
                tools=[{"type": "web_search_preview"}],  # web search
            )
            ...
            logger.error(
                f"Failed to fetch/parse LLM events: {e}\n"
                f"Response: {locals().get('response', None)}"
            )
            return []
        ...
        for event_item in parsed_output.events:
            ...
            try:
                ...
                event_time = datetime.fromisoformat(
                    event_item.event_time.replace("Z", "+00:00")
                )
                if event_time.tzinfo is None:  # Ensure timezone
                    event_time = event_time.replace(tzinfo=timezone.utc)
            except Exception as e_parse:
                logger.warning(
                    f"Could not parse event_time '{event_item.event_time}' "
                    f"for event {event_item.id}: {e_parse}. Skipping."
                )
                continue
            ...
            try:
                ...
                logger.info(
                    f"FindTargetEvents: Parsed event {tracked_event.id} "
                    f"at {tracked_event.event_time}"
                )
            except Exception as e_create:
                logger.warning(
                    f"Failed to create TrackedEvent from {event_item}: {e_create}"
                )

...
def main(with_signals=True):
    ...
    try:
        ...
        logger.info(f"Reddit fetch interval set to {fetch_interval} seconds from config.")
        save_interval = getattr(config, 'ui_update_interval', 5)
        while not shutdown_event.is_set():
            try:
                new_news_added = False
                ...
                for subreddit in config.reddit.subreddits:
                    try:
                        posts = scraper.fetch_subreddit_posts(
                            subreddit,
                            limit=config.reddit.max_posts_per_fetch
                        )
                ...
                new_news = [n for n in all_news if n.id not in app_repo.processed_news_ids]
                ...
                for news in new_news:
                    ...
                    for event_id, insight in results:
                        if event_id == "__global__":
                            ...
                            event = next((e for e in app_repo.events.get_all() if e.id == event_id), None)
                            if event:
                ...
                for event in app_repo.events.get_all():
                    ...
                    if not event.is_locked and (et_ts - now_ts) < 0:
                        ...
                        app_repo.portfolio.set(VirtualPortfolio(current_value=portfolio_manager.get_value()))

...
class RedditConfig(BaseModel):
    ...
    max_posts_per_fetch: int = Field(
        4, description="Maximum number of posts to fetch per interval"
    )
...
class PortfolioConfig(BaseModel):
    ...
    points_per_correct: float = Field(
        100.0, description="Points awarded for correct predictions"
    )
    points_per_incorrect: float = Field(
        -50.0, description="Points deducted for incorrect predictions"
    )
...
class SentimentConfig(BaseModel):
    ...
    min_confidence: float = Field(
        0.6, description="Minimum confidence score for predictions"
    )
...
class AppConfig(BaseModel):
    ...
    ui_update_interval: int = Field(
        5, description="Interval (in seconds) between UI/state updates"
    )

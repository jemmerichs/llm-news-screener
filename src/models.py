...
class NewsItem(BaseModel):
    ...
    source: str = Field(
        ..., description="Source of the news item (e.g., subreddit name)"
    )
    ...
    timestamp: datetime = Field(
        ...
        description="When the news item was created/fetched",
    )
    added_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="When the news item was added to the repository",
    )
...
class Insight(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the insight was generated"
    )
...
class TrackedEvent(BaseModel):
    ...
    event_time: datetime = Field(
        ..., description="When the event is scheduled to occur"
    )
    ...
    current_sentiment_score: float = Field(
        default=0.0, description="Current sentiment score based on news analysis"
    )
    predicted_action: Optional[str] = Field(
        None, description="Predicted action (Put/Call/Hold)"
    )
    thinking_text: Optional[str] = Field(
        None, description="Generated text explaining the current analysis"
    )
    is_locked: bool = Field(
        default=False, description="Whether the event's bias is locked"
    )
    lock_time: Optional[datetime] = Field(
        None, description="When the event's bias was locked"
    )
    insights: List[Insight] = Field(
        default_factory=list, description="LLM-generated insights for this event"
    )
...
class VirtualPortfolio(BaseModel):
    ...
    current_value: float = Field(default=1000.0, description="Portfolio value")

    class Config:
        json_schema_extra = {"example": {"current_value": 1250.75}}
